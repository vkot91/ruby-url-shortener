...
- [X] T004 Write `backend/docker-compose.yml` with postgres:17 and redis:7.4 — backing services only; the Rails server, Sidekiq, and the Next.js dev server run natively (quickstart.md)
...
- [X] T009 Set `maxmemory` and `maxmemory-policy allkeys-lru` in the `backend/docker-compose.yml` redis service, and put Sidekiq on a separate Redis database index (Principle IV, research.md D9)
...
**Checkpoint**: `docker compose up -d` plus `bin/rails s` and `pnpm dev` yield a running stack; `bundle exec rspec` and `pnpm test` pass with zero tests; `rubocop` and `eslint` pass clean and are blocking in CI.
...
- [X] T040 [P] [US1] Deterministic 10 000-link corpus generator in `load/seed.rb`
- [X] T041 [US1] k6 script with Zipf-distributed code selection in `load/redirect.js` — this exact file is reused unchanged in 3C (research.md D13)
- [X] T042 [P] [US1] Document the reference environment (host spec, compose profile, k6 flags) in `load/README.md`
- [X] T043 [US1] Run k6 against the naive build and commit results to `load/results/naive.json`
- [X] T044 [US1] Write baseline analysis in `load/results/naive-analysis.md` naming the actual bottleneck — connection pool exhaustion, per-click insert, or query latency

**Checkpoint 3B**: A committed number and a named bottleneck. **Do not proceed without these files committed.** ✅ 2026-09-01 — **1 100 redirects/second**, p99 within SC-001's 50 ms only up to 500/s. The bottleneck is neither of the two the plan expected to be able to blame: the pool never queued (6 Postgres connections against a `max_connections` of 100, zero `ConnectionTimeoutError`) and Postgres ran at 28% of one core. It is the work done per request inside a single GVL-bound Ruby process, pinned at 107% CPU — four SQL statements per redirect, of which the two that record the click are ~62% of the SQL time and none of them is needed before the response.

Three things landed alongside 3B that the task list did not name:

- `load/env.sh`, `load/seed.sh`, `load/serve.sh` — the reference environment as scripts rather
  than as prose. Principle II's "the two runs differ only in the code under test" is a claim
  about the *next* run as much as this one, and a README cannot enforce a thread count.
- `load/corpus.json` — generated by T040 and committed. The k6 script cannot reach Postgres, so
  the codes and their popularity ranks have to cross between the two halves of the harness in a
  file.
- The results file is a `handleSummary` export, not `--out json=`. That flag writes one line per
  metric sample — gigabytes, uncommittable. quickstart.md's load-test section was corrected to
  match.
...
- [ ] T068 [P] [US2] Sign-up and sign-in pages in `frontend/src/app/(auth)/`, posting to the BFF handlers from T118 rather than to Rails directly (design.md §6.2, canvas 01–02 — note the single indistinguishable failure message required by FR-036)
...
- [X] T122 [P] [US2] Shared presentation components from design.md §7 — `<ShortLink>`, `<CopyButton>`, `<ClickCount>`, `<EmptyState>` — in `frontend/src/components/`, with component tests for the two-second copy success swap and for the tabular-figure column holding width across 0, 8, and 4051 (design.md P2, P3, P4)
- [ ] T069 [US2] Link list page showing short URL, destination, created date, click count in `frontend/src/app/(dashboard)/links/page.tsx` — table columns, hover, skeleton, empty, error, and pagination per design.md §6.3, built from T122's components (canvas 03–05)
...
- [ ] T070 [US2] Link creation form with one-click copy in `frontend/src/app/(dashboard)/links/new/page.tsx` — react-hook-form + zod, mapping all six server `error.code` values to field-level errors. Copy for all six, plus the in-place success panel, is fixed in design.md §6.4 (canvas 06–07)
...
- [ ] T080 [P] [US3] Link edit form in `frontend/src/app/(dashboard)/links/[id]/edit/page.tsx` with the code field rendered read-only, reusing the zod schema from T070 (design.md §6.5, canvas 08 — a read-only code row, not a disabled input)
- [ ] T081 [P] [US3] Delete action with confirmation in `frontend/src/app/(dashboard)/links/[id]/page.tsx` — dialog copy stating the retention and 30-day reservation rules per design.md §6.6 (canvas 09)
...
- [ ] T084 [P] [US4] Safety warning page with report control in `backend/app/views/pages/banned.html.erb` (FR-018) — layout, report control, and CSS subset per design.md §6.9 and §8 (canvas 15)
...
- [ ] T095 [P] [US4] Admin link search, accounts, and health pages in `frontend/src/app/admin/` (design.md §6.8, canvas 12)
