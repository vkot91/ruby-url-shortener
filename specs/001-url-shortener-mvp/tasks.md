...
- [ ] T120 [P] Apply the design tokens from design.md §3 — replace the `:root` and `.dark` blocks in `frontend/src/app/globals.css`, set `--radius` to `0.75rem`, add `--input-border` and `--success`, add `--primary-hover`, and bind Figtree and JetBrains Mono via `next/font/google` in `frontend/src/app/layout.tsx` (design.md §3, §4, §5.2)
